...
let ataquesJugador = ["Arañazo", "Ascuas", "Corte furia", "Malicioso", "Reflejo", "Maldición","Agilidad"];
...
function activarAgilidad() {
  ...
  contadorAgilidad++;
  velocidadJugador= Math.round(Math.min(velocidadOriginalJugador*1.3**4, velocidadJugador*1.3));
}
...
if(porcentajeDefRival===100){
  dañoInfligidoPorJugador=dañoInfligidoPorJugador*(porcentajeDefRival/100);
}else{
dañoInfligidoPorJugador=dañoInfligidoPorJugador - ((dañoInfligidoPorJugador*(porcentajeDefRival-100)/100));
}
  ...
  if (tipoAtaqueJugador !== tipoJugadorPrimero) {
    ...
    if (esSuperEfectivo(tipoAtaqueJugador, tipoRivalPrimero)) {
      ...
      barSaludRivalPrimero = Math.max(0, barSaludRivalPrimero - dañoInfligidoPorJugador * 2);
      // Asegurarse de que no baje de 0
      actualizarBarraSaludRival();
      console.log("La salud del rival es: " + barSaludRivalPrimero);
    } else {
      //SI NO HAY VENTAJA...
      barSaludRivalPrimero = Math.max(0, barSaludRivalPrimero - dañoInfligidoPorJugador);
    ...
    if (esSuperEfectivo(tipoAtaqueJugador, tipoRivalPrimero)) {
      ...
      //SI NO HAY VENTAJA...
      barSaludRivalPrimero = Math.max(0, barSaludRivalPrimero - dañoInfligidoPorJugador * 1.5);
      redondeadoRival = Math.round(barSaludRivalPrimero);
      barSaludRivalPrimero = redondeadoRival;
      // Asegurarse de que no baje de 0
      actualizarBarraSaludRival();
...
if(porcentajeDefJugador===100){
  dañoInfligidoPorRival= Math.max(1, dañoInfligidoPorRival*(porcentajeDefJugador/100));
}else{
  dañoInfligidoPorRival= Math.max(1, dañoInfligidoPorRival - ((dañoInfligidoPorRival*(porcentajeDefJugador-100))/100));
}

  if (NoEsMuyEfectivo(tipoAtaqueRival, tipoJugadorPrimero)) {
    console.log("No es muy efectivo...");
    barSaludJugadorPrimero = Math.max(0, barSaludJugadorPrimero - dañoInfligidoPorRival / 2);
    ...
    barSaludJugadorPrimero = Math.max(0, barSaludJugadorPrimero - dañoInfligidoPorRival);
    redondeadoJugador = Math.round(barSaludJugadorPrimero);
    barSaludJugadorPrimero = redondeadoJugador;
    actualizarBarraSaludJugador();
  }
...
function comprobarPorcentajeDefJugador(){
  return porcentajeDefJugador=Math.floor((defJugador/defJugadorBase)*100);
}
//----------------------------------------------------------------------------------------
