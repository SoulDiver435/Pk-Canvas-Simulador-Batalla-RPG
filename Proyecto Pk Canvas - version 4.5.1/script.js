...
let botonesAtaque = document.querySelectorAll(".botonataque, #botonObjetos");
let palabrasComprobar = ["enemigo", "usó", "enemigo se debilitó"];
//Velocidad
let velocidadJugador = 8;
let velocidadRival = 15;
//Turnos
...
let maliciosoRivalEstaActivado = false;
let contadorMaliciosoRival = 0;
...
let listaDeObjetos = [objPocion];
//-------------------------------------------------------------------------------------------

//FUNCIONES DEL SISTEMA---------------------------------------------------------------------------------------------------------------------------------

//FUNCION PARA ACTUALIZAR LOS OBJETOS INDIVIDUALMENTE
function actualizarObjetos(posicionObjeto) {
  if (listaDeObjetos.length > 0) {
    if (listaDeObjetos[posicionObjeto] === objPocion) {
      if (objPocion.cantidad > 0 && objPocion.colocado === false) {
        objPocion.colocado = true;
        let objetoAñadido = document.createElement("div");
        let contListaObjetos = document.getElementById("contListaObjetos");
        objetoAñadido.className = "contObjeto";
        objetoAñadido.innerHTML = `Poción x${objPocion.cantidad}`;
        objetoAñadido.id = "objPocion";
        contListaObjetos.appendChild(objetoAñadido);

        if (listaDeObjetos.length > 0) {
          if (objPocion.cantidad > 0) {
            let usarObjPocion = document.getElementById("objPocion");
            usarObjPocion.addEventListener("click", () => {
              barSaludJugadorPrimero += 20;
              actualizarBarraSaludJugador();
              objPocion.cantidad--;
              usarObjPocion.innerHTML = `Poción x${objPocion.cantidad}`;
              contVentanaObjetos.style.display = "none";
              jugadorYaAtaco = true;
              turnoActual= 1;
              actualizarMsj(`¡El Entrenador usó Poción!`);
            });
          }
        }
      } else if (objPocion.cantidad === 0) {
        let objPocionEliminar = document.getElementById("objPocion");
        objPocionEliminar.remove();
      }
    }
  }
};

//FUNCION PARA ACTUALIZAR LA LISTA DE OBJETOS
function actualizarListaObjetos() {
  actualizarObjetos(0);
};

//FUNCIONES PARA MOSTRAR Y CERRAR VENTANA DE OBJETOS
let btnMostrarObjetos = document.getElementById("botonObjetos");
let contVentanaObjetos = document.getElementById("contVentanaObjetos");
let btncerrarVentanaObjetos = document.getElementById("btnCerrarObj");

btnMostrarObjetos.addEventListener('click', () => {
  contVentanaObjetos.style.display = "inline-block";
  actualizarListaObjetos();
  toggleBotonesAtaque(true);
});

btncerrarVentanaObjetos.addEventListener('click', () => {
  contVentanaObjetos.style.display = "none";
  toggleBotonesAtaque(false);
});


//FUNCION PARA CAMBIAR COLOR DE BARRA DE SALUD
function cambiarColorBarraSalud() {
  const barraSaludJugadorHTML = document.getElementById("barraSaludJugador");
  const barraSaludRivalHTML = document.getElementById("barraSaludRival");
  ...
  if (porcentajeJugador < 60 && porcentajeJugador > 30) {
    barraSaludJugadorHTML.style.backgroundColor = "#ffba3a";
  } else if (porcentajeJugador < 30) {
    barraSaludJugadorHTML.style.backgroundColor = "#ff3a2c";
  }else if(porcentajeJugador>60){
    barraSaludJugadorHTML.style.backgroundColor = "greenyellow";
  }

  if (porcentajeRival < 60 && porcentajeRival > 30) {
    barraSaludRivalHTML.style.backgroundColor = "#ffba3a";
  } else if (porcentajeRival < 30) {
    barraSaludRivalHTML.style.backgroundColor = "#ff3a2c";
  }}

//OBSERVAR CAMBIOS EN TAMAÑO DE BARRAS DE SALUD
const observadorBarrasSalud = new ResizeObserver((entries) => {
  for (let entry of entries) {
    cambiarColorBarraSalud(entry.target);
  }
});

//OBSERVAR LAS BARRAS DE SALUD

const barraSaludJugadorHTML = document.getElementById("barraSaludJugador");
const barraSaludRivalHTML = document.getElementById("barraSaludRival");

observadorBarrasSalud.observe(barraSaludJugadorHTML);
observadorBarrasSalud.observe(barraSaludRivalHTML);

//FUNCION PARA ACTIVAR MALICIOSO DEL RIVAL
function activarMaliciosoRival() {
  if (contadorMaliciosoRival < 4) {
    maliciosoRivalEstaActivado = true;
    defJugador = Math.floor(defJugador - (defJugador * 10) / 100);
    comprobarPorcentajeDefJugador();
  }
}

//ACTIVAR CONTADOR DE MALICIOSO RIVAL
let indiceContadorMaliciosoRival = 0;
function activarContadorMaliciosoRival() {
  if (indiceContadorMaliciosoRival < 4) {
    indiceContadorMaliciosoRival++;
    contadorMaliciosoRival = indiceContadorMaliciosoRival;
  }
}

//COMPROBAR CONTADOR MALICIOSO RIVAL
function comprobarContadorMaliciosoRival() {
  if (contadorMaliciosoRival === 4) {
    contadorMaliciosoRival = 0;
  }
}
...
function comprobarGolpeCriticoActivado() {
  ...
  golpeCriticoEstaActivado = numeroAleatorio <= 0.2;
...
//ACTIVA ATAQUE MALICIOSO DEL RIVAL
function maliciosoRival() {
  console.log("¡" + pokeRivalPrimero + " usó " + ataquesRival[4] + "!");
  tipoAtaqueRival = tiposGlobal[3];
  actualizarMsj(
    "¡" +
      pokeRivalPrimero +
      ...
      "!" +
      "<br>" +
      `¡Defensa de ${pokeJugadorPrimero} disminuyó!`
  );
  activarMaliciosoRival();
  activarContadorMaliciosoRival();
  comprobarPorcentajeDefJugador();
}
...
//OBTENER UN ATAQUE ALEATORIO DEL RIVAL
...
function actualizarBarraSaludJugador() {
  ...
  if(barSaludJugadorPrimero > maxSaludJugadorPrimero){
    barraSaludJugador = document.getElementById("barraSaludJugador");
    porcentajeJugador = 100;
    barraSaludJugador.style.width = `${porcentajeJugador}%`;
    barSaludJugadorPrimero = maxSaludJugadorPrimero;
    litsaludjugador = document.getElementById("litsaludjugador");
    litsaludjugador.innerText = barSaludJugadorPrimero + " / " + maxSaludJugadorPrimero;
  }
}
...
function comprobarContadorReflejo() {
  if (reflejoJugadorEstaActivado === true) {
    if (contadorReflejo > 0) {
      Math.max(0, contadorReflejo--);
    }
    if (contadorReflejo === 0) {
      defJugador = Math.round(
        defJugador - defJugador / (5 * (defJugador * 1.5))
      );
...
btnStatus.addEventListener("click", () => {
  ...
  console.log(`Malicioso Rival activado: ${maliciosoRivalEstaActivado}`);
  console.log(`Contador Malicioso Rival: ${contadorMaliciosoRival}`);
